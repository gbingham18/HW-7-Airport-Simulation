		...
		if (!the_queue.empty()) {
			...
			if (plane->ready_takeoff_time == -1) { // new plane has arrived at the front
				...
				if (clock - plane->ready_takeoff_time > departure_time) {
					...
					total_wait = departure_time * the_queue.size();
					//Wait time for planes entering the departure queue depends on the number of planes in the queue and the 
					//time it takes for each plane to take off, hence queue size * departure time.

					// FIXME: update total_wait and num_served
					total_wait = total_wait - departure_time;
					num_served++;
