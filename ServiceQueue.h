	...
	void update(int clock) 
	{
		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				landing_queue->total_wait = (clock / num_served) * landing_queue->thequeue.size();
				//In order to calculate the projected wait time for a plane entering the landing queue,
				//we need to divide the total time that has passed and divide it by the total number of planes
				//that have been served/ moved through the landing queue, this will give us an average service time 
				//for any given plane, we then must multiply this my the current number of planes in the queue
				//to get the average waiting time for a given plane in the service queue.
				...
				//The updated total wait is based on how long the previous plane took for servicing. 
				...
				*/
				int i = my_random.next_int(max_service_time);
				do
				{
					if (i >= min_service_time)
					{
						plane->service_time = i;
					}
				} while (i < min_service_time);
